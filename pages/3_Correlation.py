...
with dc1:
    ...
    fig_s1=px.scatter(s,x='Income_num',y='Budget_num',color='City_tier',opacity=0.55,
    ...
    _x=s['Income_num'].clip(0,120).values; _y=s['Budget_num'].values
    _m,_b=np.polyfit(_x,_y,1); _xr=np.linspace(_x.min(),_x.max(),100)
    fig_s1.add_trace(go.Scatter(x=_xr,y=_m*_xr+_b,mode='lines',
                                line=dict(color='#064e3b',width=2,dash='dash'),name='OLS trend'))
